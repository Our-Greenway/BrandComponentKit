function highlightCode(code: string, lang: string): string {
  ...
  if (lang === 'js') {
    const escaped = code.replace(/</g, '&lt;').replace(/>/g, '&gt;');
    const highlighted = escaped
      .replace(/(\/\/.*$|\/\*[\s\S]*?\*\/)/gm, `<span class="text-gray-500">$1</span>`) // comments - added multiline flag
      .replace(/("(?:[^"\\]|\\.)*")|('(?:[^'\\]|\\.)*')|(`(?:[^`\\]|\\.)*`)/g, `<span class="text-green-400">$&</span>`) // strings - fixed non-capturing groups
      .replace(/\b\d+(?:\.\d+)?\b/g, `<span class="text-yellow-300">$&</span>`) // numbers - fixed non-capturing group
      .replace(/\b(?:const|let|var|if|else|for|while|do|function|return|import|from|export|default|class|extends|super|new|try|catch|finally|throw|switch|case|break|continue|typeof|instanceof|in|of|await|async|static|get|set)\b/g, `<span class="text-purple-400">$&</span>`) // keywords - used non-capturing group
