  ...
  <div className="mx-auto max-w-10xl xl:pl-40 lg:pl-10 md:pl-10 md:p-30">
    ...
    <div className=" flex items-center ">
      <div className="relative group flex flex-col">
        ...
        {/* Logo Container */}
        <div className="scale-[2] origin-top-left relative">
          {/* Static greyed-out SVG (always visible) */}
          <img src="/GrayLogo.svg" alt="Static Logo" className="w-full h-auto" />

          {/* Colored interactive SVG (initially transparent, only specific parts are shown on hover) */}
          <svg
        ...
        {/* Info sections container - positioned below the scaled logo */}
        <div className="mt-32 relative min-h-[120px] w-96">
          {/* Info sections, hidden by default, shown based on hover */}
          <div className={`absolute top-0 left-0 w-full transition-opacity duration-300 bg-white p-4 rounded-lg shadow-lg ${hovered === "G" ? "opacity-100" : "opacity-0 pointer-events-none"}`}>
            <h2 className="font-bold text-lg text-blue-600 mb-2">"G" Shape</h2>
            <p className="text-gray-700">The "G" represents Greenway and wholeness. This curved letter symbolizes the continuous flow of nature and the interconnected pathways that bind our communities together.</p>
          </div>

          <div className={`absolute top-0 left-0 w-full transition-opacity duration-300 bg-white p-4 rounded-lg shadow-lg ${hovered === "Wave" ? "opacity-100" : "opacity-0 pointer-events-none"}`}>
            <h2 className="font-bold text-lg text-blue-600 mb-2">Wave</h2>
            <p className="text-gray-700">Represents watersheds and rivers that flow through our landscape. The wave element captures the dynamic movement of water systems that are vital to our ecosystem.</p>
          </div>

          <div className={`absolute top-0 left-0 w-full transition-opacity duration-300 bg-white p-4 rounded-lg shadow-lg ${hovered === "Wing" ? "opacity-100" : "opacity-0 pointer-events-none"}`}>
            <h2 className="font-bold text-lg text-green-600 mb-2">Wing</h2>
            <p className="text-gray-700">Symbolizes flight and freedom, representing the birds and wildlife that depend on greenway corridors for migration and habitat connectivity.</p>
          </div>

          <div className={`absolute top-0 left-0 w-full transition-opacity duration-300 bg-white p-4 rounded-lg shadow-lg ${hovered === "Bike" ? "opacity-100" : "opacity-0 pointer-events-none"}`}>
            <h2 className="font-bold text-lg text-orange-600 mb-2">Bicycle</h2>
            <p className="text-gray-700">Represents sustainable transportation and recreation. The bike element emphasizes our commitment to creating pathways for active, eco-friendly community engagement.</p>
          </div>
        </div>
      </div>
    </div>
